...
<body class="bg-gray-100 font-sans transition-colors duration-500">
    ...
    <div class="flex min-h-screen">
        ...
        <div
            class="flex flex-1 flex-col overflow-x-hidden {{ session('sidebar_collapsed', false) ? 'lg:ml-16' : 'lg:ml-72' }} transition-all duration-300 ease-in-out">
    ...
    <script>
        const checkSidebarState = () => {
            ...
            const NavigationText = document.querySelectorAll('.navigation-bar-text')
            const textappear = document.getElementById("textappear")
            console.log('isCollapsed', textappear)
            ...
            console.log('Current Width Class:', sidebar.classList.contains('w-16') ? 'w-16 (collapsed)' :
                'w-72 (expanded)');
            console.log('TranslateX Class:', sidebar.classList.contains('-translate-x-full') ?
                '-translate-x-full (hidden on mobile)' : 'translate-x-0 (visible)');
            console.log('Session/Cookie Source (approx):', sessionStorage.getItem('sidebar_collapsed') ||
                'N/A (check network tab for cookies)');
